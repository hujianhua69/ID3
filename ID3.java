...
public class ID3 {
	...
	public DecisionTree getDecisionTree(
			List<AttributeSet> attributeSets   //属性列表
			)
	{
		...
		nodes.offer(dt.root);  //决策树的根进入队列
		
		filteredAttributeSets.offer(attributeSets);
		do {
			
			DecisionTreeNode dtn = nodes.poll(); //出队列
			List<AttributeSet> correspondingAttributeSet = filteredAttributeSets.poll(); //出队列
			//通过计算,获得最大信息增益的属性
